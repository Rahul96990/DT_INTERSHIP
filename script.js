...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelector(".top-arrow").addEventListener("click", function () {
        document.getElementById("sidebar-1").classList.toggle("expanded");
    });

    // Set main title
    document.querySelector("#main #top-section h1").textContent = data.title;

    // Set explore section content
    document.querySelector("#main #explore #explore-info").innerHTML = `
        <h1>${data.tasks[0].task_title}</h1>
        <p>${data.tasks[0].task_description}</p>`;

    let containers = document.querySelectorAll("#main #containers > div");

    data.tasks[0].assets.forEach((asset, index) => {
        if (containers[index]) {
            containers[index].querySelector("#box-header h1").textContent = asset.asset_title;
            containers[index].querySelector("#description").innerHTML = `
                <p><span>Description :</span> ${asset.asset_description}</p>`;

            // If the asset is a video, embed it
            if (index === 0 && asset.asset_content_type === "video") {
                containers[index].querySelector("#youtube-link").innerHTML = `
                    <iframe width="480" height="291" src="${asset.asset_content}" frameborder="0"
